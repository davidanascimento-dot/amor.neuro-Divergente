document.addEventListener('DOMContentLoaded', () => {
    ...
    presetBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            customAmount.value = 'R$ ' + selectedValue.replace('.', ',');
        });
    });

    customAmount.addEventListener('input', () => {
        presetBtns.forEach(b => b.classList.remove('active'));
        let val = customAmount.value.replace(/\D/g, '');
        val = (parseInt(val) / 100).toFixed(2).replace('.', ',');
        customAmount.value = 'R$ ' + val;
        selectedValue = val.replace(',', '.');
    });

    btnDonate.addEventListener('click', () => {
        donationForm.style.display = 'none';
        checkoutBox.style.display = 'block';
        checkoutValueText.textContent = 'Sua doação de R$ ' + selectedValue.replace('.', ',') + ' faz toda a diferença.';
    });
    ...
    console.log('🚀 Doações inicializado!');
});